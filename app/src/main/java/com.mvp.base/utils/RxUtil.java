...
import com.mvp.base.model.net.ApiException;
import com.mvp.base.model.net.BmobHttpResponse;
import com.mvp.base.model.net.VideoHttpResponse;
...
public class RxUtil {
    ...
    public static <T> Observable.Transformer<BmobHttpResponse<T>, T> handleBmobResult() {   //compose判断结果
        return new Observable.Transformer<BmobHttpResponse<T>, T>() {
            @Override
            public Observable<T> call(Observable<BmobHttpResponse<T>> httpResponseObservable) {
                return httpResponseObservable.flatMap(new Func1<BmobHttpResponse<T>, Observable<T>>() {
                    @Override
                    public Observable<T> call(BmobHttpResponse<T> tBmobHttpResponse) {
                        if(tBmobHttpResponse.getResults() != null) {
                            return createData(tBmobHttpResponse.getResults());
                        } else {
                            return Observable.error(new ApiException("服务器返回error"));
                        }
                    }
